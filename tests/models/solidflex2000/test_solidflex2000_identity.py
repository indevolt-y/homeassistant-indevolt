"""SolidFlex2000 main-device and battery-pack identity contracts."""
...
def test_solidflex2000_sensor_uses_its_config_entry_identity() -> None:
    ...
    assert entity.unique_id == "solidflex2000-entry_142"
    assert entity.device_info == {
        "identifiers": {(DOMAIN, "SOLIDFLEX2000-SN")},
        "name": f"{MODEL} (SOLIDFLEX2000-SN)",
        "manufacturer": "INDEVOLT",
        "sw_version": "2.0.0",
        "model": MODEL,
        "serial_number": "SOLIDFLEX2000-SN",
    }


def test_solidflex2000_controls_share_its_config_entry_identity() -> None:
    ...
    assert [entity.unique_id for entity in entities] == [
        "solidflex2000-entry_backup_soc",
        "solidflex2000-entry_work_mode",
        "solidflex2000-entry_light",
    ]
    assert all(
        entity.device_info["identifiers"] == {(DOMAIN, "SOLIDFLEX2000-SN")}
        for entity in entities
    )


@pytest.mark.parametrize(("pack_id", "serial_key", "sensor_key"), PACK_CASES)
def test_solidflex2000_battery_pack_has_stable_identity_and_parent(
    pack_id,
    serial_key,
    sensor_key,
) -> None:
    pack_serial = f"SOLID-PACK-{pack_id}-SN"
    ...
    assert entity.device_info == {
        ...
        "via_device": (DOMAIN, "SOLIDFLEX2000-SN"),
    }


def test_solidflex2000_battery_pack_without_serial_is_unavailable() -> None:
    coordinator = FakeCoordinator()
    entity = IndevoltBatterySensorEntity(
        coordinator,
