"""SolidFlex2000 Home Assistant platform-registration contract."""
...
@pytest.mark.asyncio
async def test_solidflex2000_registers_its_own_platform_contract(
    monkeypatch,
) -> None:
...
@pytest.mark.asyncio
async def test_solidflex2000_exposes_real_time_number_boundary(monkeypatch) -> None:
    coordinator = FakeCoordinator()
    entry = SimpleNamespace(data={"device_model": MODEL}, runtime_data=coordinator)
    descriptions = []
    monkeypatch.setattr(
