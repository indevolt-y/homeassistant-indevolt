"""PowerFlex2000 config-flow contract."""
...
@pytest.mark.asyncio
async def test_powerflex2000_creates_its_model_entry(monkeypatch) -> None:
    ...
    result = await flow.async_step_user({"host": "192.0.2.24", "scan_interval": 45})

    assert result == {
        "type": "create_entry",
        "title": "INDEVOLT PowerFlex2000 (192.0.2.24)",
        "data": {
            "host": "192.0.2.24",
            "port": DEFAULT_PORT,
            "scan_interval": 45,
            "sn": "POWERFLEX2000-SN",
            "device_model": "PowerFlex2000",
            "fw_version": "1.2.3",
        },
    }
    assert unique_ids == ["POWERFLEX2000-SN"]
    assert duplicate_checks == [True]
    assert api_arguments == [("192.0.2.24", DEFAULT_PORT, "fake-session")]
