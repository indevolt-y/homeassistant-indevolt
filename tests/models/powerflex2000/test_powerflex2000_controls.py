"""PowerFlex2000 number, select, switch, and Action contracts."""
...
class FakeAPI:
    """Record PowerFlex2000 control writes."""
...
class FakeCoordinator:
    """Provide PowerFlex2000 control state and refresh observation."""
...
@pytest.mark.asyncio
async def test_powerflex2000_number_rejects_10801_before_write() -> None:
    coordinator = FakeCoordinator()
    description = next(item for item in NUMBERS_GEN2 if item.key == "power_setting")
    entity = make_number_entity(coordinator, description)
...
@pytest.mark.asyncio
async def test_powerflex2000_select_ignores_unknown_option() -> None:
...
@pytest.mark.asyncio
async def test_powerflex2000_select_maps_existing_option() -> None:
    coordinator = FakeCoordinator({"7101": 1})
    entity = IndevoltSelectEntity(coordinator, SELECTS_GEN2[0])
...
@pytest.mark.asyncio
async def test_powerflex2000_switch_optimistically_updates_then_writes(
    monkeypatch,
) -> None:
...
def test_powerflex2000_switch_with_null_point_is_unavailable() -> None:
...
class FakeServices:
    """Store PowerFlex2000 service handlers."""
...
@pytest.mark.asyncio
@pytest.mark.parametrize("power", [2_400, 2_401, 4_800, 7_200, 10_800])
async def test_powerflex2000_action_accepts_supported_power(
    monkeypatch,
    power,
) -> None:
    coordinator = FakeCoordinator()
...
@pytest.mark.asyncio
async def test_powerflex2000_action_rejects_10801_before_registry_or_api(
    monkeypatch,
) -> None:
    coordinator = FakeCoordinator()
    hass = make_hass(coordinator)
