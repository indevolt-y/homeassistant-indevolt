"""BK1600Ultra config-flow contract."""
...
@pytest.mark.asyncio
async def test_bk1600_ultra_creates_its_model_entry(monkeypatch) -> None:
    ...
    result = await flow.async_step_user({"host": "192.0.2.22", "scan_interval": 45})

    assert result == {
        "type": "create_entry",
        "title": "INDEVOLT BK1600/BK1600Ultra (192.0.2.22)",
        "data": {
            "host": "192.0.2.22",
            "port": DEFAULT_PORT,
            "scan_interval": 45,
    ...
    assert unique_ids == ["BK1600-ULTRA-SN"]
    assert duplicate_checks == [True]
    assert api_arguments == [("192.0.2.22", DEFAULT_PORT, "fake-session")]
