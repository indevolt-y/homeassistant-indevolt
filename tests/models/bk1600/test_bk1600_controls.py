"""BK1600 number, select, and Action contracts."""
...
class FakeCoordinator:
    """Provide BK1600 control state and refresh observation."""
...
async def test_bk1600_number_keeps_dynamic_boundary(state, maximum) -> None:
...
@pytest.mark.asyncio
async def test_bk1600_select_ignores_unknown_option() -> None:
...
@pytest.mark.asyncio
async def test_bk1600_select_maps_existing_option() -> None:
    coordinator = FakeCoordinator({"6001": 1001})
    entity = IndevoltSelectEntity(coordinator, SELECTS_GEN1[0])
...
@pytest.mark.asyncio
async def test_bk1600_action_keeps_existing_selector_maximum(monkeypatch) -> None:
