"""Unmatched-model config-flow contract."""
...
@pytest.mark.asyncio
async def test_unmatched_model_keeps_its_reported_name(monkeypatch) -> None:
    ...
    assert result == {
        "type": "create_entry",
        "title": "INDEVOLT FutureModel (192.0.2.25)",
        "data": {
            "host": "192.0.2.25",
            "port": DEFAULT_PORT,
            "scan_interval": 45,
    ...
    assert unique_ids == ["FUTURE-MODEL-SN"]
    assert duplicate_checks == [True]
    assert api_arguments == [("192.0.2.25", DEFAULT_PORT, "fake-session")]
